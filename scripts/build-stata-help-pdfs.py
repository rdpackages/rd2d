...
def find_stata(user_value: str | None) -> Path:
    ...
    for name in ("StataMP-64.exe", "StataSE-64.exe", "StataBE-64.exe", "stata-mp", "stata-se", "stata"):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Stata help PDFs from rd2d .sthlp files.")
    ...
    for stem in HELP_FILES:
        pdf = repo_root / "stata" / f"{stem}.pdf"
